...
namespace AgOpenGPS
{
    public class CYouTurn
    {
        ...
        public bool isManualTriggered = false;
        ...
        {
            vec3 pt;
            double step = Math.Min(mf.tool.toolWidth / 5, mf.vehicle.wheelbase / 5);
            int length = (int)(youTurnStartOffset / step);
            for (int a = 0; a < length; a++)
            {
                pt.easting = ytList[0].easting + (Math.Sin(head) * step);
                pt.northing = ytList[0].northing + (Math.Cos(head) * step);
            ...
            {
                pt.easting = ytList[count - 1].easting + (Math.Sin(head) * i * step);
                pt.northing = ytList[count - 1].northing + (Math.Cos(head) * i * step);
                pt.heading = head;
                ytList.Add(pt);
            }
        ...
        public bool BuildABLineDubinsYouTurn(bool isTurnRight)
        {
            ...
            if (youTurnPhase == 0)
            {
                ...
                CDubins.driveDistance = Math.Min(mf.tool.toolWidth / 50, mf.vehicle.wheelbase / 50);
                ...
                switch (mf.vehicle.turnCase)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        CDubins.turningRadius = (mf.tool.halfToolWidth - mf.tool.toolOverlap + mf.tool.toolOffset * fac)+.01;
                        break;
        ...
        public void BuildManualYouTurn(bool isTurnRight, bool isTurnButtonTriggered)
        {
            ...
            //CDubins.turningRadius = Math.Max(mf.vehicle.minTurningRadius+ (mf.vehicle.trackWidth / 2), (mf.vehicle.wheelbase*1.0/(Math.Tan(glm.toRadians(mf.vehicle.maxSteerAngle)))) + (mf.vehicle.trackWidth/2));

            //if (isTurnRight) fac = -1;
            //CDubins.turningRadius = Math.Max(CDubins.turningRadius, mf.tool.halfToolWidth - mf.tool.toolOverlap + mf.tool.toolOffset*-fac);
        ...
        public bool DistanceFromYouTurnLine()
        {
            ...
            if (ptCount > 0)
            {
                if (mf.isStanleyUsed)
                {
                    ...
                    double goalPointDistance = mf.vehicle.ulookgain * mf.vehicle.UpdateGoalPointDistance();
                    double tempLook = glm.Distance(ytList[2], ytList[3]);
                    tempLook = goalPointDistance / tempLook;
                    tempLook = Math.Round(tempLook);
                    if (tempLook < 5) tempLook = 5;

                    //find the closest 2 points to current fix
                    for (int t = lastA; t < ptCount; t++)
                    {
                    ...
                    double abHeading = ytList[B].heading;
                    abHeading = Math.Atan2(ytList[B].easting - ytList[A].easting, ytList[B].northing - ytList[A].northing);
                    ...
                    steerAngleYT = Math.Atan((distanceFromCurrentLine * mf.vehicle.stanleyDistanceErrorGain) / ((mf.avgSpeed * 0.277777) + .01));
                ...
                else
                {
                    ...
                    //goalPointYT.easting = ytList[B].easting;
                    //goalPointYT.northing = ytList[B].northing;



                    for (int i = lastA; i < ptCount && i >= 0; i += count)
                    {
                    ...
                    ppRadiusYT = (goalPointDistanceSquared) / (mf.vehicle.usteerGain * 2 * (((goalPointYT.easting - pivot.easting) * Math.Cos(localHeading)) + ((goalPointYT.northing - pivot.northing) * Math.Sin(localHeading))));
                    string temp = "";
                    steerAngleYT = glm.toDegrees(Math.Atan(mf.vehicle.usteerGain * 2 * (((goalPointYT.easting - pivot.easting) * Math.Cos(localHeading))
                        + ((goalPointYT.northing - pivot.northing) * Math.Sin(localHeading))) * mf.vehicle.wheelbase / (goalPointDistanceSquared)));
                    temp = "Origingal: " + steerAngleYT.ToString("N2");
                    if (ppRadiusYT<0) steerAngleYT = glm.toDegrees(Math.Atan((mf.vehicle.wheelbase) / (ppRadiusYT + mf.vehicle.trackWidth / 2)));
                    else steerAngleYT = glm.toDegrees(Math.Atan((mf.vehicle.wheelbase) / (ppRadiusYT - mf.vehicle.trackWidth / 2)));
                    temp += " Modified: " + steerAngleYT.ToString("N2");
                    

                    //double maxturn = glm.toDegrees(Math.Atan(((mf.vehicle.wheelbase)) / (mf.vehicle.minTurningRadius + (mf.vehicle.trackWidth / 2))));
                    //if (steerAngleYT < -maxturn) steerAngleYT = -maxturn;
                    //if (steerAngleYT > maxturn) steerAngleYT = maxturn;
                    if (steerAngleYT < -mf.vehicle.maxSteerAngle) steerAngleYT = -mf.vehicle.maxSteerAngle;
                    if (steerAngleYT > mf.vehicle.maxSteerAngle) steerAngleYT = mf.vehicle.maxSteerAngle;
