        ...
        public void GetCurrentABLine(vec3 pivot, vec3 steer)
        {
            ...
            else
            {
                ...
                steerAngleAB = glm.toDegrees(Math.Atan(2 * (((goalPointAB.easting - pivot.easting) * Math.Cos(localHeading))
                    ...
                    / goalPointDistance));
                if (ppRadiusAB < 0) steerAngleAB = glm.toDegrees(Math.Atan((mf.vehicle.wheelbase) / (ppRadiusAB + mf.vehicle.trackWidth / 2)));
                else steerAngleAB = glm.toDegrees(Math.Atan((mf.vehicle.wheelbase) / (ppRadiusAB - mf.vehicle.trackWidth / 2)));

                //double maxturn = glm.toDegrees(Math.Atan(((mf.vehicle.wheelbase)) /(mf.vehicle.minTurningRadius + (mf.vehicle.trackWidth / 2))));
                //if (steerAngleAB < -maxturn) steerAngleAB = -maxturn;
                //if (steerAngleAB > maxturn) steerAngleAB = maxturn;
