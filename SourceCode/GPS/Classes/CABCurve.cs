            ...
            if (ptCount > 0)
            {
                ...
                else// Pure Pursuit ------------------------------------------
                {
                    ...
                    steerAngleCu = glm.toDegrees(Math.Atan(2 * (((goalPointCu.easting - pivot.easting) * Math.Cos(localHeading))
                        + ((goalPointCu.northing - pivot.northing) * Math.Sin(localHeading))) * mf.vehicle.wheelbase / goalPointDistance));
                    if (ppRadiusCu < 0) steerAngleCu = glm.toDegrees(Math.Atan((mf.vehicle.wheelbase) / (ppRadiusCu + mf.vehicle.trackWidth / 2)));
                    else steerAngleCu = glm.toDegrees(Math.Atan((mf.vehicle.wheelbase) / (ppRadiusCu - mf.vehicle.trackWidth / 2)));
