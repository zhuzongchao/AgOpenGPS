...
namespace AgOpenGPS
{
    public partial class FormMap : Form
    {
        ...
        private void ImageChk_CheckedChanged(object sender, EventArgs e)
        {
            pictureBox1.Visible = ImageChk.Checked;
            ImageButton.Visible = ImageChk.Checked;
            mapControl.Visible = !ImageChk.Checked;
        }
        ...
        private void ImageButton_Click(object sender, EventArgs e)
        {
                ...
                double scale = bmpRate.Width / 500;
                
                    pictureBox1.Image = bmpRate;
                    btnN.Enabled = true;
                    btnE.Enabled = true;
                    btnS.Enabled = true;
                    btnW.Enabled = true;
                //using (Bitmap bitmap = Properties.Resources.z_output)
                {
                    GL.GenTextures(1, out mf.texture[22]);
                    GL.BindTexture(TextureTarget.Texture2D, mf.texture[22]);
                    BitmapData bitmapData = bmpRate.LockBits(new Rectangle(0, 0, bmpRate.Width, bmpRate.Height), ImageLockMode.ReadOnly, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
                    GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, bitmapData.Width, bitmapData.Height, 0, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte, bitmapData.Scan0);
                    bmpRate.UnlockBits(bitmapData);
                    GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, 9729);
                    GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, 9729);
                }
